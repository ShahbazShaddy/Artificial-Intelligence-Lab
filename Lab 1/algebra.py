# Problem 4: Algebraic simplification
...
def do_multiply(expr1, expr2):
    if isinstance(expr1, Product) and isinstance(expr2, Product):
        # Combine all factors from both products
        return Product(expr1 + expr2)
    
    elif isinstance(expr1, Sum) and isinstance(expr2, Product):
        # Distribute each term in the sum by the product
        result = []
        for term in expr1:
            # Convert term to a Product if it's not already one
            if not isinstance(term, Product):
                term = Product([term])
            result.append(Product(term + expr2))
        return Sum(result)
    
    elif isinstance(expr1, Product) and isinstance(expr2, Sum):
        # Distribute each term in the product by the sum
        result = []
        for term in expr2:
            # Convert term to a Product if it's not already one
            if not isinstance(term, Product):
                term = Product([term])
            result.append(Product(expr1 + term))
        return Sum(result)
    
    else:
        # If either expr1 or expr2 is not a Sum or Product, treat it as a single term
        return Product([expr1]) * Product([expr2])